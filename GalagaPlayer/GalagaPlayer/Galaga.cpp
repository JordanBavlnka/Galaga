...
void Galaga::play(sf::RenderWindow *window)
{
  ...
  while (window->isOpen())
  {
	...
	while (window->isOpen())
    {
	  Bee* beeptr = new Bee(paths[0]);//test enemy
	  enemies.push_back(beeptr);

	  beeptr->setHome(85, 100);

	  Butterfly* buttptr = new Butterfly(paths[1]);//test enemy
	  enemies.push_back(buttptr);

	  buttptr->setHome(105, 68);

	  Boss* bossptr = new Boss(paths[2]);//test enemy
	  enemies.push_back(bossptr);

	  bossptr->setHome(85, 45);

	  Bee* beeptr2 = new Bee(paths[3]);//test enemy
	  enemies.push_back(beeptr2);

	  beeptr2->setHome(85,116);
