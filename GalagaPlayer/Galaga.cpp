...
void Galaga::tick(sf::RenderWindow *window, int originKeyState, int numTicks, bool shoot)
{
	for (int i = 0; i < numTicks; i++)
	{
		...
		{
			tick(window, originKeyState, 40, false);
			lives--;
			if (lives >= 0)
			{
				for (int j = lives * 16; j < 133; j++)
				{
					player.rebirth(j);
			...
			for (int j = 0; j < enemies.size()&&i2==i; j++)
			{
				...
				if (collide(temp1, temp2, temp[1][0], temp[1][1], true)) {
					...
					else
					{
						...
						if (enemies.at(j)->identity() == 4)
						{
							if (enemies.at(j)->isDropping())
								score += beeScore * 2;
							else
								score += beeScore;
						}
						else if (enemies.at(j)->identity() == 3)
						{
							if (enemies.at(j)->isDropping())
